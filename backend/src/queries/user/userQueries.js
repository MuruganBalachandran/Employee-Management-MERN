...
const updateUserProfile = async (user = {}, payload = {}) => {
  try {
    ...
    throw new Error("Error while perform update user profile", err);
  }
};
...
const deleteUserAccount = async (target = {}) => {
  try {
    ...
    throw new Error("Error while perform delete user account");
  }
};
// endregion
...
const findUserById = async (id = "") => {
  try {
    ...
    throw new Error("Error while find user by id", err);
  }
};
